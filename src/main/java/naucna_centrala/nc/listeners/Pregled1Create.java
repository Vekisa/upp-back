...
@Service
public class Pregled1Create implements TaskListener {
    @Autowired
    ...
    public void notify(DelegateTask delegateTask) {
        ...
        for(FormField formField : formFields) {
            ...
            if(formField.getId().equals("naslov")) {
                Map<String,String> aaa = formField.getProperties();
                aaa.put("value",labor.getNaslov());
            }else if(formField.getId().equals("abstract")){
                Map<String,String> aaa = formField.getProperties();
                aaa.put("value",labor.getSazetak());
            }else if(formField.getId().equals("kljucne_reci")){
                Map<String,String> aaa = formField.getProperties();
                aaa.put("value",labor.getKljucniPojmovi());
            }
        }
    }
}
